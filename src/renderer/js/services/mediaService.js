...
export async function playTemporaryTrack(track) {
    ...
    try {
        ...
        if (resolved) {
            // =========================================================================
            // 【核心修改】将 isVip 和 originalTrackInfo 传递给状态
            // =========================================================================
            const playableTrack = {
                ...track,
...
export async function deleteTrack(index) {
    ...
    try {
        ...
        if (isDeletingCurrent) { mutations.setIsPlaying(false); mutations.clearPlayingTrackInfo(); }
        mutations.removeTrack(index);
        showToast(`"${track.title}" 已删除`);
        if (getters.playlist().length === 0) { setTimeout(() => window.location.reload(), 1500); return; }
